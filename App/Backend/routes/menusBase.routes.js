...
router.post('/', async (req, res) => {
    ...
    if (!verificarAlimento)
        return res.status(400).send('Ocurrió un error con el alimento seleccionado :c')
...
router.put('/:id', async (req, res) => {
    ...
    const menuBaseEditar = await MenusBase.findOneAndUpdate(req.params.id, {
        titulo: req.body.titulo,
        imagen: req.body.imagen,
        ingredientes: req.body.ingredientes,
        categoria: req.body.categoria
    ...
    if (!menuBaseEditar)
        return res.status(404).send('No se pudo editar el menú base u ocurrió algún error inesperado')
